...
<cfset StartYear = 'dateformat(todaysdate, "yyyy")'>
<cfset EndYear = 'dateformat(todaysdate, "yyyy")'>

<cfinclude template="../queries/qrysales.cfm">

<cfif qrySales.recordCount IS 0>
<div align="center">
  <h2><strong>There are no completed orders on file!</strong></h2>
</div>
<cfabort>
</cfif>
...
<cfoutput>
    <tr>
      <td width="25%">Total of Sales<br>
      to Date: <u><cfif request.EnableEuro IS 'Yes'>#lseurocurrencyformat(TotalSales, "Local")#<cfelse>#lscurrencyformat(TotalSales, "Local")#</cfif></u></td>
      <td width="25%">Total Sales<br>
      to Date: <u><font size="2" face="Verdana">#NumberOfSales#</u></td>
      <td width="25%">First Order <br>
      Placed on: <u>#DateFormat(FirstOrder, "mm/dd/yyyy")#</u></td>
      <td width="25%">Last Order <br>
      Placed on: <u>#DateFormat(LastOrder, "mm/dd/yyyy")#</u></td>
    </tr>
</cfoutput>
</table>
<form method="POST" action="doorders.cfm">
  <hr>
  <p align="left">Month and a Year of Report: 
    <select size="1" name="ReportMonth">
      <option Value='99' <cfif ThisReportMonth IS '99'>SELECTED</cfif>>All 
      Months</option>
    ...
    <input type="hidden" value="ViewSales" name="action">
    <input type="submit" value="Generate Report" name="B1">
  </p>
  <hr>
  <p align="center"><cfoutput><cfif NOT ThisReportMonth IS '99'>
...
<cfif ThisReportMonth IS '12'><cfset TheMonth = 'December'></cfif>
  ...
  <div align="center"></cfoutput>
    <center>
      <table border="0" cellpadding="6" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="85%" id="AutoNumber2">
        <tr bgcolor="#CCCCCC"> 
...
<form action="printsalesreport.cfm" method="post" name="form1" target="_blank">
  <p align="center"> 
