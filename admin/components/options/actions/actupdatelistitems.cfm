...
<cfloop index="MyCount" FROM="1" TO="#ListLen(TempVar.OptionNames, '^')#">
	...
	<cfif Evaluate(ItemFormName) IS ''>
		...
		<cfif ISDEFINED('form.itemid')>
			<cflocation url = "dooptions.cfm?Action=AddDropDown&EditErrorLine=#ErrorLine#&EditErrorMessage=#ErrorMessage#&itemid=#form.itemid#">
		<cfelse>		
			<cflocation url = "dooptions.cfm?Action=AddDropDown&EditErrorLine=#ErrorLine#&EditErrorMessage=#ErrorMessage#">
		</cfif>
	</cfif>
