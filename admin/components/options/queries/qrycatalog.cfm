...
<cfif request.dbtype IS 'mysql'>
	<cfif searchquery IS 'null'>
		...
		<cfif TempVar.SortOption IS 'OutOfStock'>WHERE products.unitsinstock < 1 AND products.isoption = 0
		<cfelseif TempVar.SortOption IS 'Featured'>
        WHERE IsFeatured = 'Yes' AND products.isoption = 0
        <cfelseif TempVar.SortOption IS 'Inactive'>
        WHERE product_categories.categoryid = 0 AND products.isoption = 0
        <cfelseif TempVar.SortOption IS 'All'>
        WHERE products.isoption = 0
        <cfelse>
        <!---otherwise select the category called--->
        WHERE product_categories.categoryid = #tempvar.sortoption# AND products.isoption = 0
        </cfif>
...
<cfif request.dbtype IS 'msaccess'>
	<cfif searchquery IS 'null'>
        ...
        SELECT * FROM products WHERE isoption = 0
        </cfquery>
		...
		<cfif TempVar.SortOption IS 'OutOfStock'>WHERE products.unitsinstock < 1 AND len(products.subof) < 1
		<cfelseif TempVar.SortOption IS 'Featured'>
