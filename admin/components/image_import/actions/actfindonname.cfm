...
SELECT productname, itemid FROM products
</cfquery>
...
<!---loop over the products and use the productname to find files that contain the productname number--->
<cfset image_files = "">
...
<cfloop query = "qryProducts">
	...
	<!---check only files that are valid image formats...if it contains the productname then add it to this one--->
    <cfset imagecount = 1>
    <cfloop query="qryFiles">
		...
		OR right(qryFiles.name, 4) IS 'jpeg'>
				...
				<cfif qryFiles.name CONTAINS '#qryProducts.productname#' OR qryProducts.productname CONTAINS '#filenoext#'>
